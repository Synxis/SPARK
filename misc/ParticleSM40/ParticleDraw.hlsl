...
struct VSParticleDrawOut
{
	...
	float4 color		: COLOR;
	float2 param		: PARAM; // size and angle
};
...
VSParticleDrawOut VSParticleDraw(VSParticleIn input)
{
	...
	output.pos = g_bufParticle[input.id].paPosition;
	output.color = g_bufParticle[input.id].paColor;
	output.param.x = g_bufParticle[input.id].paParamSize;
	output.param.y = g_bufParticle[input.id].paParamAngle;
...
void GSParticleDraw(point VSParticleDrawOut input[1], inout TriangleStream<GSParticleDrawOut> SpriteStream)
{
	...
	if( input[0].param.y != 0.0f )
	{
		float2 sc;
		sincos(input[0].param.y, sc.x, sc.y);
		rot._11 = rot._22 = sc.y;
		rot._12 = -sc.x;
		rot._21 = sc.x;
	}
	rot = mul(rot, (float3x3)g_mInvView);
	...
	for(int i=0; i<4; i++)
	{
		float3 position = g_positions[i] * input[0].param.x; //g_fParticleRad;
		
		//position = mul( position, (float3x3)g_mInvView ) + input[0].pos;
		position = mul( position, rot ) + input[0].pos;
