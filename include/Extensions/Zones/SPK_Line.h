	...
	*/
	...
	public :
		...
		*/
        ...
        const Vector3D& getBound0() const { return getBound(0); }
        const Vector3D& getBound1() const { return getBound(0); }
    ...
    public:
        spark_description(Line, Zone)
        (
            spk_attribute(Pair<Vector3D>, line, setBounds, getBound0, getBound1);
        );
	...
	inline bool Line::contains(const Vector3D& v, float radius) const
	{
		return false;
	}

	inline bool Line::intersects(const Vector3D& v0, const Vector3D& v1, float radius, Vector3D* normal) const
	{
		return false;
	}
